...
model.add(tf.layers.conv2d({filters: 3, kernelSize: 8, strides: 1, padding: 'same'}))
model.add(tf.layers.conv2d({filters: 3, kernelSize: 4, strides: 1, padding: 'same'}))
...
async function trainBatch(model, colordir, trainFileList, groupNum) {
    let batch = getRandomBatchAe(colordir, trainFileList, epochBatchSize, imageWidth, imageHeight)
